...
import WbAddressFields, {
  emptyWbAddress,
  type WbAddressValue,
} from "@/components/WbAddressFields";
import {
  formatWbAddressLabel,
  normalizeWbAddress,
...
function buildPublicDonorVerifyUrl(origin: string, donorUserId: string) {
  ...
  return `${base}/blood/donor/u/${donorUserId}`;
}
...
export default function DonorDashboardPage() {
  ...
  async function load() {
    ...
    const donorSelectWithPin =
      "user_id,name,blood_group,pincode,district,block,panchayat,village,last_donation_date,contact_number,id_card_verified,photo_object_path,preferred_days,preferred_time_slots,pause_until";
    const donorSelectBase =
    ...
    let donorErrMsg: string | null = null;

    const withPin = await supabase
      .from("donors")
      .select(donorSelectWithPin)
      .eq("user_id", user.id)
      .single();

    if (!withPin.error && withPin.data) {
      donorRow = withPin.data as DonorRow;
    } else if ((withPin.error?.message ?? "").toLowerCase().includes("pincode")) {
      const fallback = await supabase
        .from("donors")
        .select(donorSelectBase)
        .eq("user_id", user.id)
        .single();
      donorRow = (fallback.data as DonorRow | null) ?? null;
      donorErrMsg = fallback.error?.message ?? null;
    } else {
      donorErrMsg = withPin.error?.message ?? "Unable to load donor record.";
    }

    if (!donorRow) {
      setError(donorErrMsg ?? "Unable to load donor record.");
      setLoading(false);
      return;
    }

    const row = donorRow as DonorRow;
    setDonor(row);
    setPhotoUrl(null);
  ...
  async function persistDonorAddress(userId: string) {
    const supabase = getSupabaseOrNull();
    if (!supabase) {
      return { ok: false as const, error: "Supabase is not configured." };
    }

    const addr = normalizeWbAddress(address);
    const addressValidation = validateWbAddressForSearch(addr, {
      requirePanchayat: true,
    });
    if (!addressValidation.ok) {
      return {
        ok: false as const,
        error: addressValidation.error ?? "Please complete your West Bengal address.",
      };
    }

    const addressUpdate: Record<string, string> = {
      district: addr.district.trim(),
      block: addr.block.trim(),
      panchayat: addr.panchayat.trim(),
      village: addr.village.trim(),
    };
    if (addr.pincode.trim()) {
      addressUpdate.pincode = addr.pincode.trim();
    }
    ...
    if (updErr?.message.toLowerCase().includes("pincode")) {
      const { pincode: _p, ...withoutPincode } = addressUpdate;
      const retry = await supabase.from("donors").update(withoutPincode).eq("user_id", userId);
      updErr = retry.error;
    }
    if (
      updErr?.message.toLowerCase().includes("village") &&
      updErr.message.toLowerCase().includes("column")
    ) {
      const { village: _v, ...withoutVillage } = addressUpdate;
      const retry = await supabase.from("donors").update(withoutVillage).eq("user_id", userId);
      updErr = retry.error;
    }
  ...
  async function onSaveAddress() {
    ...
    const result = await persistDonorAddress(user.id);
    setSavingAddress(false);
    ...
    await load();
    window.setTimeout(() => setAddressSaved(false), 4000);
  }
  ...
  async function onSave(e: React.FormEvent) {
    ...
    if (editingAddress) {
      const addressResult = await persistDonorAddress(user.id);
      if (!addressResult.ok) {
        setSaving(false);
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .infoHead{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}
  .infoHeadText{flex:1;min-width:0}
  .qrTop{flex-shrink:0;margin-top:2px}
  ...
  .qrBox{
    border:1px solid #e2e8f0;border-radius:14px;background:#fff;padding:8px;
    display:flex;align-items:center;justify-content:center;
    box-shadow:0 4px 12px rgba(15,23,42,.06);
  }
  .qrBox img{width:88px;height:88px;display:block}
  .note{font-size:11px;color:#475569;line-height:1.5}
  .note b{color:#0f172a}
  ...
  <div class="wrap">
    <div class="card">
      ...
      <div class="body">
        ...
        <div class="info">
          <div class="infoHead">
            <div class="infoHeadText">
              <p class="name">${safeName}</p>
              <div class="uid">${safeEmail}</div>
            </div>
            ${
              qrDataUrl
                ? `<div class="qrTop"><div class="qrBox"><img src="${qrDataUrl}" alt="Scan for donor profile"/></div></div>`
                : ""
            }
          </div>
          <div class="meta">
      ...
      <div class="foot">
        ...
        <div class="note"><b>Verification:</b> Scan the QR code to open the full donor profile on Raktodaan.com.</div>
      </div>
    </div>
  ...
  async function renderIcardCanvas(): Promise<HTMLCanvasElement | null> {
    ...
    mount.innerHTML = `
      <style>
        ...
        .ic-infoHead{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}
        .ic-infoHeadText{flex:1;min-width:0}
        .ic-qrTop{flex-shrink:0;margin-top:2px}
      ...
      <div class="ic-wrap">
        <div class="ic-card">
          ...
          <div class="ic-body">
            ...
            <div class="ic-info">
              <div class="ic-infoHead">
                <div class="ic-infoHeadText">
                  <p class="ic-name">${safeName}</p>
                  <div class="ic-uid">${safeEmail}</div>
                </div>
                ${
                  qrDataUrl
                    ? `<div class="ic-qrTop"><div class="ic-qrbox"><img src="${qrDataUrl}" alt="Scan for donor profile"/></div></div>`
                    : ""
                }
              </div>
              <div class="ic-meta">
...
<b style="color:#0f172a">Verification:</b> Scan the QR to open the full donor profile on Raktodaan.com.
          </div>
        </div>
      </div>
    `;
    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-10">
      <div className="rounded-3xl border border-rose-100 bg-gradient-to-b from-rose-50/70 via-white to-white p-6 shadow-sm backdrop-blur">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {!editingAddress ? (
                <button
                  ...
                  className="rounded-lg border border-zinc-200 bg-white px-3 py-1 text-xs font-semibold text-zinc-800 hover:bg-zinc-50"
                >
                  Edit address
                </button>
              ) : null}
        ...
        <form className="mt-6 grid gap-4 sm:grid-cols-2" onSubmit={onSave}>
          {editingAddress ? (
            <div className="sm:col-span-2 rounded-xl border border-rose-200/50 bg-rose-50/30 p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="text-sm font-semibold text-zinc-900">Update address</div>
                <button
                  type="button"
                  onClick={onCancelAddressEdit}
                  className="text-xs font-semibold text-zinc-600 underline underline-offset-2 hover:text-zinc-900"
                >
                  Cancel
                </button>
              </div>
              <p className="mt-1 text-xs text-zinc-600">
                Use PIN or post office search (West Bengal only). Changes apply when you save.
              </p>
              <div className="mt-4">
                <WbAddressFields
                  value={address}
              ...
              {addressError ? (
                <p className="mt-2 text-xs text-rose-700">{addressError}</p>
              ) : null}
              <div className="mt-4">
                <button
                  type="button"
                  ...
                  className="rounded-xl bg-gradient-to-r from-red-600 to-rose-500 px-4 py-2 text-sm font-semibold text-white shadow-sm disabled:opacity-60"
                >
          ...
          {addressSaved ? (
            <p className="sm:col-span-2 text-sm font-medium text-emerald-700">
              Address saved. Find Donors search will use your updated location.
            </p>
          ) : null}
