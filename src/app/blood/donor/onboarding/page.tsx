...
export default function DonorOnboardingPage() {
  ...
  useEffect(() => {
    // Use static WB_DISTRICTS as the primary source for dropdown options.
    ...
    // Optionally still load blood_groups from Supabase if available.
    const supabase = getSupabaseOrNull();
    if (!supabase) {
  ...
  async function loadPanchayatsForBlock(nextBlockId: string) {
    ...
    const district = WB_DISTRICTS.find(
      (d) => d.district === districtId,
    );
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const payloadWithVillage = {
        ...
        panchayat: selectedPanchayat.panchayat_name,
        village,
    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-10">
      <div className="rounded-2xl border bg-white/70 p-6 shadow-sm backdrop-blur">
        ...
        <form className="mt-6 grid gap-4 sm:grid-cols-2" onSubmit={onSubmit}>
          ...
          {dropdownLoadError ? (
            <div className="sm:col-span-2 -mt-2 rounded-xl border border-rose-200 bg-rose-50/70 p-3 text-sm text-rose-700">
              {dropdownLoadError}
            </div>
          ) : null}
          ...
          !districtsLoading &&
          districts.length === 0 ? (
            <div className="sm:col-span-2 -mt-2 rounded-xl border border-amber-200 bg-amber-50/70 p-3 text-sm text-amber-900">
              Location dropdowns are empty. Import West Bengal
              District/Block/Panchayat data into Supabase tables
              (`districts`, `blocks`, `panchayats`) before registering.
            </div>
          ) : null}
